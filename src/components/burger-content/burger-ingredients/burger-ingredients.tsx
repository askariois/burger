...
function BurgerIngredients() {
  const { feed } = useSelector((state: IIngredients) => state.ingredient);
      ...
      <div className={`${burgeringredient.scroll}`}>
        <div className={`mt-10`}>
          ...
          <div className={`flex flex-wrap`}>
            {bun.map((item: IFeed | any) => <CardBurgerIngredient key={item._id} {...item} />)}
          </div>
        </div>
        <div className={`mt-10`}>
